...
function DisplayTimecards($connection, $ID){
  $totalCards =           0;
  $stmt =                 $connection->prepare("SELECT * FROM lineas WHERE ConsultorID=?");
  $stmt ->                bind_param('i', $I);
  $I =                    $ID;
  $stmt ->                execute();
...
function DisplayDetails($connection, $ID){
  ...
  $query =                $connection->prepare("SELECT c.*, divisions.Name as dName, areas.Name as aName
                                                FROM consultors c
                                                INNER JOIN divisions ON(c.Division = divisions.ID)
